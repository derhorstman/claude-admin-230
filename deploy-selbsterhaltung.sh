...
# Referenz-Server für SSH-Keys (alle Keys von dort kopieren)
REFERENCE_SERVER="192.168.42.253"

# 1. SSH-Key kopieren (versuche beide Ports)
echo "[1/10] SSH-Key kopieren..."
...
echo "[4/10] Node.js 22 + tmux installieren..."
...
echo "[5/10] Claude Code 2.0.65 installieren..."
...
echo "[6/10] Verzeichnisse erstellen..."
...
if [ "$PORT" = "22" ]; then
    echo "[8/10] SSH-Port auf 2222 umstellen..."
    ...
    echo "[8/10] SSH-Port bereits 2222"
fi

# 9. Alle SSH-Keys vom Referenz-Server kopieren
echo "[9/10] SSH-Keys vom Referenz-Server kopieren..."
ssh -p 2222 $USER@$REFERENCE_SERVER "cat ~/.ssh/authorized_keys" > /tmp/all_authorized_keys 2>/dev/null
if [ -s /tmp/all_authorized_keys ]; then
    scp -P $PORT /tmp/all_authorized_keys $USER@$IP:~/.ssh/authorized_keys
    KEY_COUNT=$(wc -l < /tmp/all_authorized_keys)
    echo "    $KEY_COUNT Keys kopiert von $REFERENCE_SERVER"
    rm /tmp/all_authorized_keys
else
    echo "    WARNUNG: Konnte Keys nicht von $REFERENCE_SERVER holen"
fi

# 10. Statische IP setzen
echo "[10/12] Statische IP konfigurieren..."
SSH_CMD="ssh -o StrictHostKeyChecking=no -p $PORT $USER@$IP"
$SSH_CMD "sudo tee /etc/network/interfaces > /dev/null << 'EOFNET'
...
iface eth0 inet static
    ...
    dns-nameservers 192.168.42.216
EOFNET"
echo "    IP $IP statisch gesetzt"

# 11. SMB-Freigabe einrichten
echo "[11/12] SMB-Freigabe einrichten..."
$SSH_CMD "sudo apt-get install -y samba" 2>&1 | tail -3
$SSH_CMD "sudo tee /etc/samba/smb.conf > /dev/null << 'EOFSMB'
[global]
   workgroup = WORKGROUP
...
[dieterhorst]
   ...
   browseable = yes
   read only = no
   valid users = dieterhorst
   create mask = 0644
   directory mask = 0755
   force user = root
   force group = root
EOFSMB"
echo -e "$PASS\n$PASS" | $SSH_CMD "sudo smbpasswd -a $USER -s" 2>/dev/null
$SSH_CMD "sudo systemctl restart smbd nmbd && sudo systemctl enable smbd nmbd" 2>/dev/null
echo "    SMB-Freigabe \\\\$IP\\dieterhorst eingerichtet"

# 12. Verbindung testen
echo "[12/12] Verbindung testen..."
SSH_CMD="ssh -o StrictHostKeyChecking=no -p $PORT $USER@$IP"
